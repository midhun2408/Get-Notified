  ...
  return new Promise((resolve) => {
    ...
    https.get(url, options, (res) => {
      ...
      if (res.statusCode !== 200) {
        console.log(`[Enrichment] Failed to fetch ${url} - Status: ${res.statusCode}`);
        return resolve(null);
      }
...
async function fetchArticleData(url) {
  try {
    ...
    // 3. Handle intermediate Google pages if we didn't decode or if redirects failed
      ...
      try {
        ...
        for (const article of items) {
          ...
          if (!doc.exists) {
            ...
            // C. Wait briefly to look more human and avoid rate limits
